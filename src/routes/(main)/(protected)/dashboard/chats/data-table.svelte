<script lang="ts">
  ...
  const columns = table.createColumns([
    ...
    table.column({
      ...
      cell: ({ row }) => {
        ...
        return createRender(DataTableUser, { user: row.original.user1, read: row.original.user1Read });
      }
    }),
    ...
    table.column({
      ...
      cell: ({ row }) => {
        if (!row.isData()) return "";
        return createRender(DataTableUser, { user: row.original.user2, read: row.original.user2Read });
      }
    }),

    table.column({
      accessor: ({ _count }) => _count.messages,
      header: "Messages",
      plugins: {
        filter: {
          exclude: true
    ...
    table.column({
      ...
      cell: ({ row, value }) => {
        if (!row.isData()) return "";
        return createRender(DataTableActions, { id: value, username: row.original.user1.username + " & " + row.original.user2.username });
      },
      plugins: {
        sort: {
          disable: true
  ...
  $: $hiddenColumnIds = Object.entries(hideForId)
    .filter(([, hide]) => !hide)
    .map(([id]) => id);
  const hidableCols = ["Messages", "createdAt"];
  ...
  let chatsDeleteDialogOpen = false;
...
<div class="flex justify-center">
  <div class="w-full max-w-7xl flex-col">
    <div class="flex items-center py-4">
      <Input class="max-w-sm border-none bg-accent ring-offset-background placeholder:text-muted-foreground focus-visible:border-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-0" placeholder="Filter chats..." type="text" bind:value={$filterValue} />
      <DropdownMenu.Root>
        ...
        <DropdownMenu.Root>
          ...
          <DropdownMenu.Content class="border-border bg-popover text-popover-foreground">
            ...
            <DropdownMenu.Group>
              ...
              <DropdownMenu.Item on:click={() => (chatsDeleteDialogOpen = true)}><Trash2 class="mr-2 h-4 w-4" />Delete chats</DropdownMenu.Item>
            </DropdownMenu.Group>
          </DropdownMenu.Content>
        </DropdownMenu.Root>
      </div>
...
<AlertDialog.Root bind:open={chatsDeleteDialogOpen} closeOnEscape={!loading} closeOnOutsideClick={!loading}>
  <AlertDialog.Content>
    <AlertDialog.Header>
      <AlertDialog.Title>Delete selected chats</AlertDialog.Title>
      <AlertDialog.Description>
        This will delete all the selected chats and their messages.
        <br /><br />
